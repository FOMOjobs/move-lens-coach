...
export const Route = createFileRoute("/postepy")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Trendy jakości ruchu w czasie: powtórzenia, głębokość, Form Score.",
      },
    ],
  }),
    ...
    <div className="px-5 pt-8">
      <header className="mb-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Śledzimy <em>jakość</em> ruchu w czasie, nie tylko ilość.
        </p>
      </header>
      ...
      <section className="rounded-3xl border border-hairline bg-card p-5 shadow-sm">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Twoja średnia jakość wykonania rośnie konsekwentnie. Świetna praca.
            </p>
          </div>
        </div>
      </section>
        ...
        <ul className="mt-3 divide-y divide-hairline">
          {sessions.map((s) => (
            <li key={s.date} className="flex items-center justify-between py-3">
              ...
              <div className="rounded-full bg-tint px-3 py-1 text-sm font-semibold text-primary-deep">
                {s.score}
              </div>
            </li>
          ))}
